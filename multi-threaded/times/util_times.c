...
{
    int status = 0;
    //status = system("du -ks /home/yqhuang");
    status = system("./paster2.out");
    printf("status = %d.\n", status);
    return NULL;
};
...
int sprintf_time(void *buf, time_t t)
{
    ...
    tm_ptr = localtime(&t);
    ret = strftime(buf, BUF_SIZE_256, "time and date: %r, %a %b %d, %Y", tm_ptr);
    if (ret == 0) {
        perror("strftime");
        return -errno;
    }
